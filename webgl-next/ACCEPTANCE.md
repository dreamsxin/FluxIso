...
`npm run test:webgl` currently runs the complete fixture and lifecycle matrix in
Chromium/SwiftShader at 1280×720 and DPR 1. It rejects blank/low-variation
canvases, verifies exact pixel stability across two frames, checks fixture
controls and enabled omni light counts, and emits candidate PNG + JSON metadata.
CI uploads those files for 14 days. After human approval, the same suite will
compare against committed baselines using the 1.5% threshold above.
