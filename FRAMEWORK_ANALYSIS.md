...
| 维度 | Excalibur.js | Phaser 3 | LuxIso v1 | LuxIso v2 | LuxIso v3（当前） |
|------|:---:|:---:|:---:|:---:|:---:|
| ECS 设计 | 8/10 | 6/10 | 4/10 | 5/10 | 5/10 |
| 渲染管线 | 7/10 | 9/10 | 7/10 | 7/10 | **8/10** |
| 物理/碰撞 | 7/10 | 8/10 | 6/10 | 7/10 | 7/10 |
| 寻路 | 5/10 | 4/10 | 8/10 | 9/10 | 9/10 |
| 场景管理 | 9/10 | 8/10 | 7/10 | 7/10 | 7/10 |
| 资源管理 | 8/10 | 9/10 | 4/10 | 7/10 | **8/10** |
| 音频 | 7/10 | 8/10 | 7/10 | 7/10 | 7/10 |
| 性能（每帧 GC） | 8/10 | 7/10 | 4/10 | 8/10 | 8/10 |
| 工具链（编辑器） | 5/10 | 7/10 | 8/10 | 8/10 | **9/10** |
| 类型安全 | 9/10 | 6/10 | 7/10 | 8/10 | 8/10 |
| **综合** | **7.3** | **7.2** | **6.2** | **7.3** | **7.6** |

**v3 主要提升：**
- **渲染管线 +1**：Floor tile 等距坐标系 Bug 修复（`tileH/2` 偏移彻底消除），场景编辑器 overlay z 轴感知投影，所有可视元素现在精确对齐等距网格。
- **资源管理 +1**：`AssetLoader.register()` 支持 data URL 注入，Sprite Editor 不再依赖脆弱的 `Object.defineProperty` hack。
- **工具链 +1**：Scene Editor 补全 DirectionalLight 放置 + 可选中调整；walkable/blocked 拖动画笔；右键删除；`camera.screenToWorld` 精确拾取；Sprite Editor 修复 rebuild 死循环、每帧 DOM 查询、`anchorY` 参数传递。

等距渲染精确度、寻路、编辑器工具链方面已超越同类框架；ECS System 层、固定时间步、深度排序空间分区仍是与顶级框架的主要差距。
