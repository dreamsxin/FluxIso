...
### 第三版修复（2026-04-10）— 编辑器专项

| 问题 | 状态 | 解决方案 |
|------|------|----------|
| **Floor tile 绘制偏移 `tileH/2`** — `project(col,row)` 用作菱形中心而非左顶角 | ✅ 已修复 | 改用四角格点 `project(col,row)` / `project(col+1,row)` / `project(col+1,row+1)` / `project(col,row+1)` 构成菱形，完全对齐等距坐标系 |
| **EditorRenderer overlay 菱形高亮偏移** — `_drawTileHighlight` 同上问题 | ✅ 已修复 | 改为 `project(col+0.5, row+0.5)` 作为菱形中心 |
| **光源放置位置偏移** — `canvasToWorld` 用裸 `unproject`，zoom/pan 后不准 | ✅ 已修复 | 改用 `camera.screenToWorld()`，zoom/pan 下精确 |
| **Overlay 选中圆圈与对象不重叠** — 所有 overlay 绘制硬编码 `z=0` | ✅ 已修复 | 选中圆、标签、omnilight/dirlight anchor 均带对象自身 `z` 参与 `project()` |
| **DirectionalLight 无法选中** — `EditorLight.x/y` 对 directional 为 undefined，`findObjectAt` 跳过 | ✅ 已修复 | `EditorLight.x/y` 改为必填；directional 放置时记录点击坐标；`toJSON` 导出为 `_editorX/_editorY`，不污染 Engine schema |
| **`findObjectAt` 命中半径过小** — 0.7 世界单位，点光/方向光难以点中 | ✅ 已修复 | 扩大到 1.2 世界单位；反向遍历优先命中最上层对象 |
| **omnilight 默认 `z:120` 导致圆圈偏上 120px** | ✅ 已修复 | 默认 `z:0`；改用 `snapToTile` 落在格子中心 |
| **Sprite Editor `Object.defineProperty` hack** — 覆盖 readonly `image`/`anchorY` 运行时静默失败 | ✅ 已修复 | `AssetLoader.register(url, img)` 直接注入缓存；`anchorY` 构造时传入 |
| **Sprite Editor rebuild 死循环** — `rebuildSheet()` 调 `renderActionsList()` 再触发 input 事件 | ✅ 已修复 | 彻底解耦：name 字段改用 `change` 事件；`rebuildSheet()` 不再重建 DOM |
| **Sprite Editor 每帧 `querySelector`** — 无缓存，rebuild 后旧引用失效 | ✅ 已修复 | `Map<Direction, DirCell>` 缓存 canvas + ctx + animator |
| **`AssetLoader` 缺少 `register()` 方法** | ✅ 已修复 | 新增实例方法 + 静态委托 |
| **`DirectionalAnimator.buildSheet` 硬编码 `anchorY:1`** | ✅ 已修复 | 新增 `anchorY` 参数（默认 1，向后兼容） |
...
| 维度 | Excalibur.js | Phaser 3 | LuxIso v1 | LuxIso v2 | LuxIso v3 | LuxIso v4（当前） |
|------|:---:|:---:|:---:|:---:|:---:|:---:|
| ECS 设计 | 8/10 | 6/10 | 4/10 | 5/10 | 5/10 | 5/10 |
| 渲染管线 | 7/10 | 9/10 | 7/10 | 7/10 | 8/10 | **8/10** |
| 物理/碰撞 | 7/10 | 8/10 | 6/10 | 7/10 | 7/10 | 7/10 |
| 寻路 | 5/10 | 4/10 | 8/10 | 9/10 | 9/10 | 9/10 |
| 场景管理 | 9/10 | 8/10 | 7/10 | 7/10 | 7/10 | 7/10 |
| 资源管理 | 8/10 | 9/10 | 4/10 | 7/10 | 8/10 | 8/10 |
| 音频 | 7/10 | 8/10 | 7/10 | 7/10 | 7/10 | 7/10 |
| 性能（每帧 GC） | 8/10 | 7/10 | 4/10 | 8/10 | 8/10 | 8/10 |
| 工具链（编辑器） | 5/10 | 7/10 | 8/10 | 8/10 | 9/10 | 9/10 |
| 类型安全 | 9/10 | 6/10 | 7/10 | 8/10 | 8/10 | **9/10** |
| 遮挡正确性 | 7/10 | 7/10 | 5/10 | 6/10 | 6/10 | **8/10** |
| **综合** | **7.3** | **7.2** | **6.2** | **7.3** | **7.6** | **7.8** |

**v4 主要提升（遮挡正确性 + 测试覆盖）：**
- **遮挡正确性 +2**：修复 `isBehind` 的 containment 分支忽略 Z 轴的 Bug（高墙被错误画在矮角色后面）；统一所有对象 AABB 的 Z 单位为 `Z_UNITS_PER_PX = 1/16`（1 单位 = tileH/2 px），消除 Wall(÷16)、Character(÷8)、Cloud(原始 px)、depthSort(+1)、ShadowCaster(+2.2) 五套并存的不一致约定；`overlapZ` 跨类比较现在有意义。
- **类型安全 +1**：`Z_UNITS_PER_PX` 导出为单一真相源；AABB Z 单位约定文档化。
- **修复混合轴等远角和导致的退化循环**：`<=` 改 `<`，相等和不再产生双向边，排序顺序不再退化为输入序。
- **修复 `pairKey` 位打包溢出**：`i<<16|j` 在 j≥65536 时碰撞，改用 `i*n+j`。
- **修复光源光晕在非默认视角下错位**：`Scene.draw` 的 halo 现在正确应用 rotation/elevation 变换（此前 `project(.., view)` 忽略 view）。
- **测试覆盖 157→194**：新增 `AABB.test.ts`（Z 单位一致性）、`SceneDraw.test.ts`（渲染管线集成，首次覆盖 `Scene.draw`/LightmapCache/ShadowCaster/Floor.draw/Wall.draw 光晕路径），扩展 `depthSort.test.ts`（+7 回归用例覆盖 containment/堆叠/混合轴/orphan/链式）与 `IsoProjection.test.ts`（drawIsoCube、view 忽略行为）。

等距渲染精确度、寻路、编辑器工具链、遮挡正确性方面已达到/超越同类框架；ECS System 层、固定时间步、深度排序空间分区仍是与顶级框架的主要差距。
